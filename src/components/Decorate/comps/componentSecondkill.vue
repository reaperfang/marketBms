            ...
            <template v-if="hasRealData || hasFakeData">
                <ul v-if="hasRealData">
                    <li v-for="(item,key) of displayList" :key="key" :style="[goodMargin,goodWidth]" :class="['goodsStyle'+goodsStyle,{goodsChamfer:goodsChamfer!=1},'goodsRatio'+goodsRatio]">
                        <div class="img_box">
                            <el-image :key="listStyle" :scroll-container="listStyle == 6 ? null : '.phone-body'" :src="item.goodsImgUrl" alt="" :class="{goodsFill:goodsFill!=1}" lazy></el-image>
                        </div>
                        <div class="countdown_Bar" v-if="showContents.indexOf('5')!=-1">
                            <!-- <h1 class="title">{{item.activityName || '限时秒杀'}}</h1> -->
                            <h1 class="title">限时秒杀</h1>
                            <div class="countdown">
                        ...
                        <div class="info_box" v-if="showContents.length > 0">
                            <p class="name" :class="[{textStyle:textStyle!=1},{textAlign:textAlign!=1}]" v-if="item.skuMidGoodsLimitDiscountEtcViewList.length>0&&showContents.indexOf('1')!=-1"><font class="label">减{{getReduce(item)}}元</font>{{item.goodsName}}</p>
                            ...
                            <div class="price_line">
                                <componentButton :decorationStyle="buttonStyle" :decorationText="currentComponentData.data.buttonText" class="button" v-if="showContents.indexOf('8')!=-1&&(item.status==1||item.status==0)"></componentButton>
...
.componentSecondkill.listStyle1{
    li{
        &:first-child{
...
.componentSecondkill.listStyle2{
    ul{
        overflow:hidden;
        display:flex;
        display:-webkit-flex;
...
.componentSecondkill.listStyle3{
    ul{
        display:flex;
        display:-webkit-flex;
        flex-wrap:wrap;
...
.componentSecondkill.listStyle4{
    li{
        padding:10px 12.5px;
        position:relative;
        &:first-child{
...
.componentSecondkill.listStyle5{
    ul{
        display:flex;
        display:-webkit-flex;
        justify-content:space-between;
...
.componentSecondkill.listStyle6{
    background:#fff;
    ul{
        display:flex;
        display:-webkit-flex;
