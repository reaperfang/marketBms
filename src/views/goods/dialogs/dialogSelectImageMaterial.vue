    ...
    <el-tabs v-model="currentTab" style="margin-bottom:40px;">
      ...
      <el-tab-pane label="素材图片" name="material">
        ...
        <div
          ...
          :style="{'overflow-y': materialLoading ? 'hidden' : 'auto'}"
        >
          <template v-if="materialResultList.length">
            ...
            >
              <template>
                <div
                  class="cell-item"
                  :class="{'img_active':  materialSelectedItem && materialSelectedItem.id === item.id}"
                  v-for="(item,key) in materialResultList"
                  :key="key"
        ...
        <div class="icon_wrapper" ref="systemWrapper" v-loading="localLoading">
          <template v-if="systemResultList.length">
            ...
            <ul v-if="!localLoading">
              <li
                class="cell-item"
                :class="{'img_active': systemGroupId === systemLoadedGroupId ?  (systemRecordMap[systemGroupId] && systemRecordMap[systemGroupId].id === item.id) : (systemRecordMap[''] && systemRecordMap[''].id === item.id)}"
                v-for="(item,key) in systemResultList"
                :key="key"
                @click="selectImg(item)"
              >
      ...
      <el-tab-pane label="本地上传" name="local">
        <div
          class="material_wrapper material_wrapper_2"
          ref="localWrapper"
          :style="{'overflow-y': uploadLoading ? 'hidden' : 'auto'}"
        >
          <el-upload
            v-if="uploadAble"
            ...
            >
              <template>
                ...
                >
                  ...
                  <div class="item-img" v-if="item.isSelected">
                    <i class="alreaySelect" @click="checkImage(item)"></i>
                    <span>已选择</span>
                  </div>
                  <div class="item-img" v-if="(!item.isSelected && localSelectedItemList.length<6)">
                    <i @click="checkImage(item)">yes</i>
                    <span>选择</span>
                  </div>
                  <div class="item-img" v-if="(!item.isSelected && localSelectedItemList.length===6)">
                    <i class="forbid-sel">no</i>
                    <span>不能选择</span>
                  </div>
                </div>
              </template>
        ...
        <p class="note" style="color: #d3d8df;margin-top:10px;height: 16px;">
          仅支持jpg,jpeg,png格式，大小不超过3.0MB
          <span
            v-if="!uploadLoading && fileList.length"
            type="text"
            style="margin-left:10px;font-size:14px;color:rgb(101,94,255);cursor:pointer;"
            @click="clearTempSave"
          >清除上传记录</span>
        </p>
      </el-tab-pane>
...
export default {
  ...
  data() {
    return {
      ...
      localSelectedItemList: [], //存储本地选中的图片
  ...
  methods: {
    //改变图片的选中状态
    checkImage(data) {
      let tempList = JSON.parse(localStorage.getItem('localImageList'));
      if(tempList.length < 6){
        if (!data.isSelected) {
          tempList.push(data);
          data.isSelected = true;
        }else{
          tempList.splice(tempList.findIndex(item=>item.title===data.title),1);
      ...
      this.localSelectedItemList = tempList;
      localStorage.setItem('localImageList',JSON.stringify(tempList));
    ...
    fetchMaterial(startIndex, pageSize) {
      ...
      this._apis.file
        .getMaterialList({
          fileGroupInfoId:
            this.materialGroupId == "0" ? "" : this.materialGroupId || "",
    ...
    beforeUpload(file) {
      ...
      const isNameRight = /\.jpg|\.jpeg|\.png|\.JPG|\.JPEG|\.PNG$/.test(
        file.name
      );
      if (!(isJPG || isJPEG || isPNG) || !isLt2M || !isNameRight) {
        this.$message.error(
          "上传图片仅支持jpg,jpeg,png格式! 且上传图片大小不能超过 3MB!"
        );
        this.failedList.push(file);
        this.uploadLoading = false;
      }
      if (
        this.successList.length + this.failedList.length ===
        this.addList.length
      ) {
        this.preload(this.fileList, "url");
        this.addList = [];
    ...
    handleSuccess(response, file, fileList) {
      ...
      for (let item of fileList) {
        if (item.status == "success") {
          item.index < 6 ? (item.isSelected = true) : (item.isSelected = false);
      ...
      list.forEach((item, index) => {
        if (index < 6) {
          ...
          this.localSelectedItemList.push(item);
        } else {
      ...
      localStorage.setItem("localImageList",JSON.stringify(this.localSelectedItemList));
    },
    ...
    submit() {
      ...
      if (this.currentTab == "material") {
        ...
        // if (!this.localSelectedItem) {
          if(JSON.parse(localStorage.getItem("localImageList")).length<=0){
          this.$message.warning("请选择图片后重试！");
          return;
        }
        // copyItem = { ...this.localSelectedItem };
        copyItem={...JSON.parse(localStorage.getItem("localImageList"))}
  ...
  .cell-item {
    ...
    .item-img {
      ...
      .alreaySelect {
        ...
        border-radius: 2px;
        background-image: url("../../../assets/images/goods/check.png");
