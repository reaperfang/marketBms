    ...
    <DialogBase :visible.sync="visible" title="复制链接" width="520px" :showFooter="showFooter">
        <div>
            <div>
                <div class="url-box">
                    <ul>
                        <li v-for="item in list">{{item.url}}</li>
                    </ul>
                    <textarea style="position: absolute; left: -99999px; top: 0;" class="value-list" cols="20" rows="10" id="biao1">{{list.map(val => JSON.stringify(val)).join(',')}}</textarea>
                </div>
