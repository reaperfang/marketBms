...
export default {
	...
	methods:{
        init(){
            ...
            this.src = `${process.env.NODE_ENV === 'dev' ? '//127.0.0.1:8080' : process.env.DATA_API}/vue/marketing${this.path}?access=1&token=${this.token}&businessId=1&loginUserId=1&tenantId=${this.tenantId}&cid=${this.cid}&userName=${userName}&menuId=${applyId}&id=${applyId}&bossProductId=${bossProductId}${extraQuery}`
        },
