...
function hasPermission(msfList, route) {
  if (route && route.path) {
    ...
    return msfList.some(item => title == item.name) || route.name == 'profile' || route.path == '/401' || route.path == '/login' || route.matched[0].path == '/shop' || route.name == 'accountInfo' || route.name == 'passwordChange' || route.name == 'intelligent' || route.name == 'intelligentGuide'
  } else {
    return true
  }
}
